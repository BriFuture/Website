...
class Mail {
   ...
   * 发信人
   */
  private $from = 'noreply@mail.sunsmell.cc';

  /**
   * 发信人名称
   */
  private $proxy_name = 'sunsmell.cc的管理员';
  ...
  public function __construct() {
    $this->base = Factory::getObject('Base');
  }
  /**
   * 发送邮件
  ...
  public function send($msg) {
    ...
    $secutiry = Factory::getObject('Security');
    
    if(!ALLOW_MAIL) {
      $this->base->fatal_error('管理员关闭了邮箱功能，请联系管理员','无法发送邮件','无法发送邮件');
    }
    //先验证是否是通过正常途径发起的邮件
    if($secutiry->check_code($GLOVARS['sendmail'], @$_POST['random_code']))
    {
      if(!is_null($to))
